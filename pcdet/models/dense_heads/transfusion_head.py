...
class TransFusionHead(nn.Module):
    ...
    def __init__(
        ...
        self.heatmap_head = nn.Sequential(*layers) # all convolutions
    ...
    def predict(self, batch_dict):
        ...
        # For ease, convert the lidar feat back to its original form ,if sliced
        if 'chosen_tile_coords' in batch_dict:
            lidar_feat = scatter_sliced_tensors(
                    batch_dict['chosen_tile_coords'], [lidar_feat],
                    batch_dict['sched_algo'], batch_dict['tcount'])[0]
